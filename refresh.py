...
GROQ_KEY      = os.environ.get(‘GROQ_API_KEY’, ‘’)       # Groq AI API 키
DART_KEY      = os.environ.get(‘DART_API_KEY’, ‘’)       # DART 공시 API 키
GH_PAT        = os.environ.get(‘GH_PAT’, ‘’)             # GitHub Personal Access Token
GITHUB_REPO   = os.environ.get(‘GITHUB_REPOSITORY’, ‘’) # 배포 대상 레포지토리

# AI 모델 — 전 단계 gpt-oss-120b 단일 사용

# Groq Developer: 250,000 TPM, 슬라이딩 윈도우 98% = 245,000 목표

# call_groq(GROQ_MODEL_HIGH, system, user, max_tokens) 직접 호출

GROQ_MODEL_HIGH = ‘openai/gpt-oss-120b’  # 전 단계 동일 모델 (분기 없음)
GROQ_MODEL_FAST = ‘openai/gpt-oss-120b’  # GROQ_MODEL_HIGH와 동일 — 호환성 유지
HTML_FILE     = ‘hloomberg.html’

# AI_MODE: full=전체AI(KST 07:00~20:00), partial=종합+TOP10, ‘’=없음(매 15분 갱신)
...
AI_HOURLY = NOW.minute in range(5, 12)  # :07분 cron 기준 ±7분 허용 (15분 갱신 주기)
...
# 유틸 함수 — HTML/JS 이스케이프, 시각 포맷, HTTP 래퍼
...
“”“HTTP GET 공통 래퍼 — 실패 시 None 반환, 에러 메시지 출력.”””
try:
...
def fmt_time(dt):
“”“datetime → 상대 시각 문자열 (방금/N분전/N시간전/날짜).”””
if not dt:
return ‘’
...
# KIS REST(1차) → Yahoo Finance(2차 fallback)

# 국내: 지수(KOSPI/KOSDAQ) + 종목 9개

# 글로벌: 원유/금/환율/지수 등 TICKERS 전종목
...
PRICE_DATA = {}   # KIS 1차 → Yahoo fallback 2차 자동 처리
TICKERS = {
...
if p > 0:
nm = d.get(‘hts_kor_isnm’, ‘’).strip()
return {‘p’: p, ‘c’: c, ‘p0’: p0, ‘src’: ‘KIS’, ‘name’: nm}
...
# 이동평균 계산 (fetch_kis_ohlcv, fetch_ohlcv 공용)

def ma(arr, n):
“”“이동평균 — fetch_kis_ohlcv / fetch_ohlcv 공용. i < n-1 구간은 None 반환.”””
...
def fetch_yahoo(sym):
“”“Yahoo Finance v8 API — 5일 close 기반 현재가/등락률. 반환: {p, c, src}.”””
...
def fetch_stooq(sym):
“”“Stooq CSV fallback — Yahoo 실패 시 글로벌 지수 조회. 반환: {p, c, src}.”””
try:
...
“”“시세 조회 — fetch_yahoo → fetch_stooq 순서로 fallback.”””
...
def fmt_price(v, key):
“”“TICK_META dp 기준 가격 포맷팅. dp=0 → 정수, 그 외 소수점.”””
...
“”“원자재 차트용 — Yahoo v8에서 close 시계열만 추출. [{d, v}] 반환.”””
for q in (‘query1’, ‘query2’):
try:
...
# KIS fetch_kis_ohlcv(1차) → Yahoo fetch_ohlcv(2차)

# MA5/20/60 + RSI14 + Bollinger(20) 포함
...
“”“종목 OHLCV + MA5/20/60 + RSI14 + Bollinger(20) — Yahoo Yahoo fallback.
KIS fetch_kis_ohlcv 실패 시 사용. [{d,o,h,l,c,v,ma5,ma20,ma60,rsi,bbu,bbl}] 반환.”””
for q in (‘query1’, ‘query2’):
try:
url = f’https://{q}.finance.yahoo.com/v8/finance/chart/{requests.utils.quote(sym)}?interval=1d&range={days}d’
...
“”“FRED API — 경제지표 시계열. yoy=True 시 전년비 자동 계산.”””
...
“”“섹터 ETF 5일 close → 전일 대비 등락. 반환: {price, chg}.”””
for q in (‘query1’,‘query2’):
try:
url = f’https://{q}.finance.yahoo.com/v8/finance/chart/{requests.utils.quote(sym)}?interval=1d&range=5d’
...
# 국내: RSS 19개 + Google News 10개 → 키워드 가중치 정렬 → 상위 10건

# 해외: RSS 15개 + Google News 8개  → 관련도 점수 정렬  → 상위 10건

# 필터: BLACKLIST_KW 비경제 뉴스 제거
...
“”“RSS/Atom 파싱 → [{title,link,src,tag,tc,time,stamp}] 반환.”””
...
“”“Google News RSS — 언어·지역 지정 검색 → parse_rss 위임.”””
...
def score_news(item):
“”“국내뉴스 가중치 — stamp 기반 + KW_TIER1(+7200)/2(+5400)/3(+3600)/ECON(+1800).”””
t = item.get(‘title’,’’)
s = item.get(‘stamp’, 0)
for kw in KW_TIER1:
...
def score_gl(item):
“”“해외뉴스 가중치 — GL_SCORE_KW 5계층 × 1800점. 에너지/중동 최우선.”””
t = item[‘title’].lower()
s = item.get(‘stamp’, 0)
...
# 해외뉴스 제목 한글 번역 (gpt-oss-120b)

# ─────────────────────────────────────────

# 3. 공시 수집

# DART OpenAPI(1차) → KIND RSS fallback(2차)

# KW_TIER1/2 가중치 정렬 → 상위 10건
...
def score_dart(item):
“”“공시 가중치 — title+corp 기준 KW_TIER1(+7200)/2(+5400). stamp 미사용.”””
...
# 네이버금융 기업분석 + 투자전략 페이지 scraping → 30건
...
# [1/4] 종합분석: BIAS / FORECAST / PICKS / RISK 4섹션

# [2/4] 이슈분석: 글로벌 10개 + 국내 10개 JSON 배열

# [3/4] 원자재:   BRENT/WTI/NATGAS/GOLD 등 8종 방향성·관련주

# [4/4] 패턴A:    Step1(시그널) → Step2(수급20) → Step3(TOP10) → Step4(전략)
...
“”“AI 이슈분석 JSON 파싱 — 3단계 자동 복구 (배열추출→trailing comma→부분복구).”””
...
“”“Groq TPM 슬라이딩 윈도우 제어기.
지난 60초 누적 토큰 기준(Groq 측정 방식)으로 TPM 98% 목표 유지.
TokenBucket(충전속도 기반)은 Groq와 맞지 않아 슬라이딩 윈도우로 교체됨.
“””
# gpt-oss-120b Developer Plan: 250,000 TPM × 98% = 245,000 목표
TPM_70B   = 245000  # GROQ_MODEL_HIGH 모델 TPM 한도
TPM_8B    = 245000  # GROQ_MODEL_FAST 모델 TPM 한도 (동일 모델, 호환성)
WINDOW    = 60.0    # 슬라이딩 윈도우 60초 (Groq 측정 기준)
...
“”“해외뉴스 제목 일괄 한글 번역 — 한글 미포함 항목만 선별해 call_groq 단건 처리.”””
...
```
# KIS 국내 종목 실시간 시세 문자열 구성
kis_price_str = '\n'.join([
    f"{d.get('name','?')}({cd}): {d.get('p',0):,}원 ({'+' if d.get('c',0)>=0 else ''}{d.get('c',0):.2f}%)"
    for cd, d in (kis_stock_data or {}).items()
]) or '없음'

combined_prompt = (
    f'[분석기준] {ANALYSIS_BASE}\n\n'
    f'[글로벌시세]\n{price_str}\n\n'
    f'[국내종목실시간시세-PICKS 추천 시 이 가격 기준으로 작성]\n{kis_price_str}\n\n'
    f'[국내뉴스]\n{kr_str}\n\n'
    f'[해외뉴스]\n{gl_str}\n\n'
    f'[공시]\n{dart_str}\n\n'
    f'[리서치]\n{research_str}'
...
try:
    print('  [1/3] 종합분석...')
    t = call_groq(GROQ_MODEL_HIGH,
        '한국 증시 전문 애널리스트 겸 글로벌 매크로 전략가. 모든 외국어는 한글로 번역. 논문체로 작성(~이다, ~한다, ~됐다). 각 섹션 최소 3~5문장 이상 구체적으로 작성. 사실에 근거하여 작성할 것.',
        ...
        '### PICKS\n'
        '추천종목 3~5개: 반드시 [국내종목실시간시세] 데이터에 있는 종목만 추천. 실시간 현재가 기준으로 진입가격대·목표가·손절가 명시. 임의 종목 추천 금지.\n\n'
        '### RISK\n'
...
# 5-2. 이슈분석 (글로벌 5개)
issue_prompt = (
    '규칙: 모든 외국어는 한글로 번역. 논문체로 작성(~이다, ~한다, ~됐다).\n'
    '각 이슈는 JSON 객체: {"title":"한국어제목","impact":"상/중/하","summary":"논문체 2문장요약",'
    '"scenarioA":"낙관시나리오+확률","scenarioB":"비관시나리오+확률",'
    '"drivers":["한국어변수1","한국어변수2","한국어변수3"],"action":"한국어 투자시사점"}\n'
    'JSON 배열만 출력. 다른 텍스트 없이. [] 로 감싸서.'
)
if AI_FULL:
    try:
        print('  [2/4] 글로벌 이슈분석...')
        gt = call_groq(GROQ_MODEL_HIGH, '글로벌 매크로 전략가. 모든 외국어는 한글로 번역. 논문체로 작성(~이다, ~한다, ~됐다).',
    ...
    try:
        print('  [3/4] 국내 이슈분석...')
        dt = call_groq(GROQ_MODEL_HIGH, '한국 증시 전문 애널리스트. 모든 외국어는 한글로 번역. 논문체로 작성(~이다, ~한다, ~됐다).',
...
# 5-4. 패턴A 체인드 리서치 (4단계)
#    Step1: 뉴스·이슈 → 종목별 매수/매도 시그널 추출 (JSON)
#    Step2: 시그널 + KIS 수급 결합 → 후보 20개 선별 (JSON)
#    Step3: 리스크 필터링 (VIX·섹터·수급) → TOP10 확정 (JSON)
#    Step4: 현재가 기준 목표가·손절가 생성 → swing_quick 구성
swing_quick = {}
swing_top10 = []
if AI_FULL:
    try:
        ...
        step1_resp = call_groq(GROQ_MODEL_HIGH,
            '한국주식 시그널 추출 전문가. 모든 외국어는 한글로 번역. JSON만 출력. 다른 텍스트 없이. 반드시 뉴스에 실제로 언급된 종목만 추출할 것 — 임의 추가 절대 금지. 종목코드는 반드시 6자리 숫자, 모르면 000000으로 표시.',
            '\n'.join(filter(None, [
                f'[국내뉴스]\n' + '\n'.join([f"{i+1}. [{n['src']}] {n['title']}" for i,n in enumerate(kr_news[:15])]),
                f'[글로벌 이슈]\n{issue_summary}',
                f'[현재시세]\n{price_str}',
                f'[AI 시장전망]\n{ai_sections.get("forecast","")[:400]}' if ai_sections.get('forecast') else None,
                f'[AI 편향분석]\n{ai_sections.get("bias","")[:300]}' if ai_sections.get('bias') else None,
                '뉴스·이슈·시장전망을 종합해 종목별 매수/매도 시그널 추출. 뉴스에 실제 언급된 종목 우선.\n'
        ...
        def _fz(s): return re.sub(r'[\s\-\.]', '', s).lower()
        ...
        swing_top10 = [
            ...
            and re.fullmatch(r'\d{6}', str(x.get('code','')))
        ][:10]
...
# 캐시 키: URL or 제목 앞 80자 — 신규 항목만 call_groq 호출

# _has_foreign() 오염 감지 → 외국어/존댓말/헤더 위반 시 재생성

# 저장: summaries_cache.json (git commit 영속)
...
def summarize_news(items, existing_cache, label, system_prompt, max_new=10):
    ...
    new_count = 0
    sys_prompt = system_prompt + ' 마크다운 헤더(###,##,#) 절대 사용 금지. 단락 구분은 빈 줄로만.'
    ...
    for n in items[:max_new]:
        ...
        if key in cache:
            continue
        try:
            t = call_groq(GROQ_MODEL_HIGH, sys_prompt,
                f"제목: {n['title']}\n출처: {n.get('src','')}\n태그: {n.get('tag','')}", 1500)
...
def summarize_dart(items, existing_cache, max_new=10):
    ...
    for d in items[:max_new]:
        ...
        if key in cache:
            continue
        try:
            t = call_groq(GROQ_MODEL_HIGH, '금융공시 전문가. 모든 외국어는 한글로 번역. 논문체로 작성(~이다, ~한다, ~됐다). 마크다운 헤더 금지. 각 단락은 정확히 두 문장으로 작성. 투자자 관점 핵심 요약. 공시 원문 내용에 충실하게 요약. 임의로 내용을 만들지 말 것.',
                f"공시: {d['title']}\n기업: {d.get('corp','')}\n유형: {d.get('type','')}", 1200)
...
if GROQ_KEY and AI_FULL:
    print(f'\n[뉴스/공시 요약] 10건 처리 (gpt-oss-120b, 신규 항목만)...')
# 캐시 오염 감지 — 제약조건(외국어·존댓말·마크다운헤더) 위반 항목 제거 후 재생성
def _has_foreign(text):
    """캐시 오염 감지 — 제약조건:
    '모든 외국어는 한글로 번역. 논문체(~이다, ~한다, ~됐다). 마크다운 헤더 금지.'
    위반 시 True → 캐시 삭제 후 재생성

    허용 문자: 한글(가-힣·ㄱ-ㅎ·ㅏ-ㅣ) + 영숫자(A-Z·a-z·0-9) + 공백·줄바꿈
              + HTML 태그·속성·엔티티 + 일반 특수문자(구두점·기호)
    그 외 모든 유니코드 문자(러시아어·일본어·한자·중국어·아랍어 등) → 외국어 판정
    """
    # HTML 태그·엔티티 제거 후 텍스트만 검사
    plain = re.sub(r'<[^>]+>', ' ', text)       # HTML 태그 제거
    plain = re.sub(r'&[a-zA-Z#0-9]+;', ' ', plain)  # HTML 엔티티 제거
    # 허용 문자 패턴: 한글 + ASCII(영숫자·공백·특수) + URL 기호
    allowed = re.compile(
        r'[\uAC00-\uD7A3'  # 한글 완성형
        r'\u3131-\u318E'   # 한글 자모
        r'\u0000-\u007F'   # ASCII 전체 (영숫자·공백·특수문자 포함)
        r'\uFFFD'           # 대체문자
        r']+'
    )
    # 허용 문자를 제거한 후 남은 문자가 있으면 외국어
    foreign_chars = allowed.sub('', plain).strip()
    if foreign_chars:
        return True
    # 존댓말 패턴 (논문체 위반)
    if re.search(r'(입니다|합니다|했습니다|됩니다|있습니다|없습니다|이에요|예요)', plain):
        return True
    # 마크다운 헤더 잔존
    if re.search(r'^\s*#{1,4}\s', plain, re.MULTILINE):
        return True
    return False

def clean_cache(cache, max_size=150):
    # _has_foreign()이 마크다운 헤더 포함 전체 제약 검사
    cleaned = dict(cache)
    # 제약조건 위반 항목 제거 → 재생성 (외국어·존댓말·마크다운헤더)
    before = len(cleaned)
    cleaned = {k: v for k, v in cleaned.items() if not _has_foreign(v.get('html',''))}
    removed = before - len(cleaned)
    if removed > 0:
        print(f'  캐시 오염 제거: {removed}건 (제약조건 위반)')
    # 최대 크기 초과 시 오래된 항목 제거 (ts 기준 정렬)
    if len(cleaned) > max_size:
        sorted_items = sorted(cleaned.items(), key=lambda x: x[1].get('ts',''), reverse=True)
        cleaned = dict(sorted_items[:max_size])
    return cleaned
kr_news_summaries = clean_cache(kr_news_summaries, max_size=100)
gl_news_summaries = clean_cache(gl_news_summaries, max_size=100)
dart_summaries    = clean_cache(dart_summaries, max_size=150)

kr_news_summaries = summarize_news(kr_news, kr_news_summaries, '국내뉴스',
    '한국 금융/경제 뉴스 전문 기자. 모든 외국어는 한글로 번역. 논문체로 작성(~이다, ~한다, ~됐다). 마크다운 헤더 금지. 각 단락은 정확히 두 문장으로 작성. 배경/핵심/시장영향/투자시사점 4단락 완성. 뉴스 원문 내용에 충실하게 요약. 임의로 내용을 만들지 말 것.', max_new=10)
gl_news_summaries = summarize_news(gl_news, gl_news_summaries, '해외뉴스',
    '글로벌 금융/경제 뉴스 전문 기자. 모든 외국어는 한글로 번역. 논문체로 작성(~이다, ~한다, ~됐다). 마크다운 헤더 금지. 각 단락은 정확히 두 문장으로 작성. 배경/핵심/시장영향/투자시사점 4단락 완성. 뉴스 원문 내용에 충실하게 요약. 임의로 내용을 만들지 말 것.', max_new=10)
...
# hloomberg.html 내 ##TAG_S## / ##TAG_E## 마커 치환

# 패치 순서: TS → TICKS → KIS_PRICES → STOCKS → SUPPLY_DEMAND

# → NEWS → DART → RESEARCH → AI → ISSUES → STOCK_ANALYSIS

# ─────────────────────────────────────────

# ── PICKS 섹션을 패턴A TOP10 결과로 대체 (실시간 시세 반영)

if swing_top10 and ai_sections.get(‘picks’):
try:
# kis_stock_data 이름→코드 역조회 맵
_name_to_code = {v.get(‘name’,’?’): k for k, v in (kis_stock_data or {}).items()}
_picks_lines = []
for _s in swing_top10:
_nm = _s.get(‘name’,’’)
_cd = _s.get(‘code’,’’)
_sig = _s.get(‘act’, _s.get(‘signal’,‘관심’))
_desc = _s.get(‘desc’, ‘’)
# KIS 실시간 시세
_kd = (kis_stock_data or {}).get(_cd, {})
_price = f”{_kd.get(‘p’,0):,}원 ({’+’ if _kd.get(‘c’,0)>=0 else ‘’}{_kd.get(‘c’,0):.2f}%)” if _kd else ‘가격 미확인’
# swing_quick에서 목표가/손절가
_sq = swing_quick.get(_nm, {})
_tgt = f”{_sq.get(‘target’,0):,}원” if _sq.get(‘target’) else ‘-’
_stp = f”{_sq.get(‘stop’,0):,}원” if _sq.get(‘stop’) else ‘-’
_picks_lines.append(
f’<p style="margin:0 0 8px 0"><strong style="color:#58a6ff">’
f’{_nm}({_cd})</strong> — {_sig} ’
f’<span style="color:#aaa;font-size:12px">{_price}</span><br>’
f’<span style="font-size:13px">{_desc[:80] if _desc else “”}</span><br>’
f’<span style="font-size:12px;color:#ffa500">목표가: {_tgt} / 손절가: {_stp}</span></p>’
)
if _picks_lines:
_picks_html = ‘<div>’ + ‘’.join(_picks_lines) + ‘</div>’
ai_sections[‘picks’] = _picks_html
print(f’  PICKS: 패턴A TOP10 {len(swing_top10)}개로 대체’)
except Exception as _pe:
print(f’  PICKS 대체 실패: {_pe}’)

print(f’\n[패치] {HTML_FILE} 패치 중…’)

def patch(html, s_marker, e_marker, content):
“”“HTML 마커(##TAG_S## / ##TAG_E##) 사이 내용을 content로 치환.”””
...
# KIS_PRICES 재패치 — TOP10 신규 종목 코드 추가 조회 후 재삽입
html = patch(html, ‘// ##KIS_PRICES_S##’, ‘// ##KIS_PRICES_E##’, make_kis_prices_js(kis_stock_data))
else:
investor_data = {}
...
“”“뉴스 항목 HTML 리스트 생성 — onclick showNews(prefix, idx) 연결.”””
h = ‘’
...
“”“공시 항목 HTML 리스트 생성 — onclick showDart(idx) 연결.”””
h = ‘’
...
“”“리서치 항목 HTML 리스트 생성 — onclick showResearch(idx) 연결.”””
h = ‘’
for i, d in enumerate(items):
...
“”“요약 캐시 dict → const VAR_NAME=JSON; 형태 JS 문자열. json.dumps 직접 사용.”””
...
“”“원자재 차트 시계열 → const CMDTY_CHART={…}; JS 객체 문자열.”””
...
“”“이슈 데이터 → JS 배열 리터럴. drivers 필드는 json.dumps 별도 처리.”””
lines = [’[’]
for d in items:
...
# 7. 종목 상세분석 (STOCK_MODE=1 시 실행, 1시간 주기)

# 대상: 패턴A TOP10 우선, 없으면 STOCK_LIST 기본 9종목

# 7항목: 펀더멘털/기술적/진입전략/목표가/손절/촉매/종합의견

# KIS 재무비율 + 투자자 동향 실데이터 주입
...
def _nm(s):
# 종목명 정규화: 전각→반각, 카타카나/히라가나 제거
nm = s.get(‘name’, ‘’)
result = []
for c in nm:
code = ord(c)
if 0xFF01 <= code <= 0xFF5E: result.append(chr(code - 0xFEE0))
elif 0x3040 <= code <= 0x30FF: continue
else: result.append(c)
return ‘’.join(result).strip()
_analysis_list = [{‘name’:_nm(s),‘th’:s.get(‘th’,’’),‘mkt’:s.get(‘mkt’,‘KOSPI’),‘act’:s.get(‘act’,‘관심’),‘desc’:s.get(‘desc’,’’),‘code’:s.get(‘code’,’’)} for s in swing_top10] if swing_top10 else STOCK_LIST
...
t = call_groq(GROQ_MODEL_HIGH,
‘한국주식 전문 애널리스트. 모든 외국어는 한글로 번역. 논문체로 작성(~이다, ~한다, ~됐다). 구체적 수치와 가격 레벨 명시.’,
f”[현재시세]\n{price_str}\n”
f”{fin_str}{inv_str}\n”
...
“”“종목 상세분석 결과 → const STOCK_ANALYSIS={…}; JS 객체 문자열.”””
if not data:
...
“”“종목 OHLCV 차트 데이터 → const STOCK_CHARTS={…}; JS 객체 문자열.”””
if not data:
return ‘\nconst STOCK_CHARTS={};\n’
...
“”“FRED/BOK/섹터 데이터 → const FRED_DATA/BOK_DATA/SECTOR_DATA; JS 문자열.”””
...
# 조건 알림: 급등락(±3%) / 임팩트 상 이슈 / BRENT $110 돌파·$70 하회

# 정기 요약: 매 정각 (:00~:07) or 수동 실행 시

# 중복 방지: tg_alert_cache.json (sent_keys 기록)
...
def load_tg_cache():
“”“Telegram 알림 캐시 로드 — 중복 전송 방지용 sent_keys 목록.”””
try:
...
“”“Telegram 알림 캐시 저장 — /tmp/tg_alert_cache.json.”””
try:
...
# ── 정기 요약 (매 정각 후 7분 이내 or 수동 실행 시)
import os as _os
is_manual   = _os.environ.get('GITHUB_EVENT_NAME','') == 'workflow_dispatch'
is_hourly   = NOW.minute < 6   # :00~:07 → 15분 갱신 주기 기준

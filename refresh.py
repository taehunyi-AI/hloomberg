...
if GROQ_KEY and AI_PARTIAL:
    ...
    # 5-2. 이슈분석 (글로벌 5개)
    issue_prompt = (
        '규칙: 모든 텍스트는 반드시 순수 한국어로 작성. 한자·영어·일본어·러시아어·베트남어 등 외국어 절대 금지. 외국어 단어는 한국어로 완전히 번역할 것.\n'
    ...
    if AI_FULL:
        try:
            print('  [2/4] 글로벌 이슈분석...')
            gt = call_claude('claude-sonnet-4-20250514', '글로벌 매크로 전략가. 반드시 한국어만 사용. 외국어(한자·영어·일본어·러시아어·베트남어 등) 절대 금지 — 한국어로 완전히 번역할 것. 논문체(~이다, ~한다). 존댓말 금지.',
        ...
        try:
            print('  [3/4] 국내 이슈분석...')
            dt = call_claude('claude-sonnet-4-20250514', '한국 증시 전문 애널리스트. 반드시 한국어만 사용. 외국어(한자·영어·일본어·러시아어 등) 절대 금지 — 한국어로 완전히 번역할 것. 논문체(~이다, ~한다). 존댓말 금지.',
    ...
    if AI_FULL:
        try:
            ...
            step1_resp = call_claude(
                ...
                '한국주식 시그널 추출 전문가. 반드시 한국어만 사용. 외국어(한자·영어 혼용) 절대 금지. JSON만 출력. 존댓말 금지. 반드시 뉴스에 실제로 언급된 종목만 추출할 것 — 임의로 종목을 추가하지 말 것. 코드는 반드시 6자리 숫자여야 하며 모르면 000000으로 표시.',
                '\n'.join(filter(None, [
            ...
            step2_resp = call_claude(
                'claude-sonnet-4-20250514',
                '한국주식 포트폴리오 매니저. 반드시 한국어만 사용. 외국어(한자·영어 혼용) 절대 금지. JSON만 출력. 논문체(~이다, ~한다). 존댓말 금지. 반드시 [선택 가능 종목 목록]에 있는 종목만 선별할 것 — 목록 외 종목 추가 절대 금지. 목록에 있는 이름·코드를 그대로 사용할 것.',
                '\n'.join(filter(None, [
                    f'[Step1 시그널]\n{step1_str}',
                    f'[KIS 실시간 시세]\n{kis_price_str}',
                    f'[외국인 순매수 (실제코드 포함)]\n' + '\n'.join([
                        ...
                        try:
                            ...
                            if _sr and _sr.ok:
                                ...
                                for _item in _rj.get('output',[]): 
                                    ...
                                    if _c and _re3.fullmatch(r'\d{6}',_c):
                                        _web_code = _c
                                        _s['name'] = _n or _nm
                                        _s['code'] = _web_code
            ...
            step3_resp = call_claude(
                'claude-sonnet-4-20250514',
                '한국주식 리스크 매니저. 반드시 한국어만 사용. 외국어(한자·영어 혼용) 절대 금지. JSON만 출력. 논문체(~이다, ~한다). 존댓말 금지.',
                f'[Step2 후보]\n{step2_str}\n\n'
                f'[현재 매크로]\n'
            ...
            step4_resp = call_claude(
                'claude-haiku-4-5-20251001',
                '한국주식 트레이딩 전략가. 반드시 한국어만 사용. 외국어(한자·영어 혼용) 절대 금지. JSON만 출력. 논문체(~이다, ~한다). 존댓말 금지.',
                f'[확정 TOP10]\n{top10_str}\n\n'
                f'[KIS 실시간 시세]\n{kis_price_str}\n\n'
...
if GROQ_KEY:
    ...
    def clean_cache(cache, max_size=150):
        # 마크다운 헤더 잔존 항목 제거
        cleaned = {k: v for k, v in cache.items() if '###' not in v.get('html','') and '##' not in v.get('html','')}
        # 외국어(러시아어·일본어) 포함 항목 제거 → 재생성
        before = len(cleaned)
        cleaned = {k: v for k, v in cleaned.items() if not _has_foreign(v.get('html',''))}
        removed = before - len(cleaned)
        if removed > 0:
            print(f'  캐시 오염 제거: {removed}건 (외국어 포함)')
